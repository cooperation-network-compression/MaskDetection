...
const panelbbox = document.getElementById('panelbbox');
...
//使用AJAX
  $.ajax({
    ...
    dataType: "json"
      ...
      writeResult(response_data.results);
  });
}
...
function writeResult(results) {
  var str = '';
  for(bboxInfo of results) { 
    strbbox = bboxInfo['bbox'];
    strdetail = bboxInfo['detail'];
    str = str + strbbox + ' ';
  }
  str.replace('undefined','');
  panelbbox.innerHTML= str;
  paneldetail.innerHTML= strdetail;  
}
